...
namespace StudentCompass.Repositories.Implementations
{
    public class ProgressRepository : IProgressRepository
    {
        private readonly AppDbContext _dbContext;
        private readonly ILogger<ProgressRepository> _logger;
        public ProgressRepository(ILogger<ProgressRepository> logger, AppDbContext dbContext)
        {
            _logger = logger;
        ...
        public async Task<IEnumerable<Subject>> GetProgressOverview(short studentId, byte careerPlanId)
        {
            try
            {
                var studentIdParam = new SqlParameter("@student_id", studentId);
                var careerPlanIdParam = new SqlParameter("@career_plan_id", careerPlanId);

                var subjects = await _dbContext.Subject
                    .FromSqlRaw("EXEC app.academic_student_info @student_id, @career_plan_id", studentIdParam, careerPlanIdParam)
                    .ToListAsync();
            ...
            {
                _logger.LogError(ex, "SqlException: Error fetching progress overview for student {StudentId}", studentId);
                throw;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Unknown error fetching progress overview for student {StudentId}", studentId);
                throw;
            }
        }
